...
class CauseModelTest(TestCase):
    def test_cause_name_saved_lowercase(self):
        ...
        cause, created = Cause.create_cause(name="Education", description="A test cause")
...
class ProjectModelTest(TestCase):
    def test_create_project_with_causes(self):
        ...
        project, created = Project.create_project(name="Tree Planting", target=target, causes=[cause])
        self.assertTrue(created)
    ...
    def test_get_project_by_name(self):
        ...
        project, created = Project.create_project(name="Solar Energy", target=target)
        self.assertIsNotNone(Project.objects.filter(name="Solar Energy").first())
    ...
    def test_target_minimum_validator(self):
        ...
        valid_project = Project(name="Valid Project", target=Money(0.01, "USD"))
        try:
            valid_project.full_clean()  # Should not raise an error
        except ValidationError:
            self.fail("Project with valid target raised a ValidationError unexpectedly.")
    ...
    def test_create_project_with_money_object_target(self):
        ...
        project, created = Project.create_project(
            name="Solar Panels Expansion",
            target=amount,  # Money object
        )
...
class ProjectAssignmentTest(TestCase):
    def setUp(self):
        ...
        self.project, _ = Project.create_project(name="Water Conservation", target=self.target)
    ...
    def test_assign_beneficiary_user(self):
        ...
        assignment, created = ProjectAssignment.assign_beneficiary(self.project, self.user1)
        self.assertTrue(created)
    ...
    def test_assign_beneficiary_usergroup(self):
        ...
        assignment, created = ProjectAssignment.assign_beneficiary(self.project, self.group)
        self.assertTrue(created)
    ...
    def test_duplicate_assignment(self):
        ...
        """
        assignment1, created1 = ProjectAssignment.assign_beneficiary(self.project, self.user1)
        assignment2, created2 = ProjectAssignment.assign_beneficiary(self.project, self.user1)
    ...
    def test_unassign_beneficiary(self):
        ...
        result = ProjectAssignment.unassign_beneficiary(self.project, self.user1)
    ...
    def test_reassign_beneficiary(self):
        ...
        assignment, created = ProjectAssignment.assign_beneficiary(self.project, self.user1)
        updated = ProjectAssignment.reassign(self.project, self.user1, self.user2)
    ...
    def test_invalid_beneficiary(self):
        ...
        with self.assertRaises(ValueError):
            ProjectAssignment.assign_beneficiary(self.project, "invalid beneficiary")
